...
$JsonOutput = & $PythonCmd $PyScript db-get completed
$Tasks = @()
...
# 2. Filtra apenas a atividade das ultimas 24h
$RecentTasks = $Tasks | Where-Object {
    if ($_.completedAt) {
        $compDate = [datetime]$_.completedAt
        return $compDate -ge $Today
    }
    return $false
}

# 3. Seletividade Inteligente: Omitir relatorios inuteis
if ($RecentTasks.Count -eq 0) { exit 0 }

# 4. Formata o Relatorio Fractal e Didatico em Markdown
$ReportDir = Join-Path $ProjectRoot ".claude\reports"

if (!(Test-Path $ReportDir)) { Write-Error ".claude\reports nao encontrado"; exit 1 }

if (-not (Test-Path $ReportDir)) { New-Item -ItemType Directory -Path $ReportDir -Force | Out-Null }
$ReportPath = Join-Path $ReportDir "Cortex_Pulse_$(Get-Date -Format 'yyyyMMdd').md"

$Grouped = $RecentTasks | Group-Object agent
$Md = @()
$Md += "# [CORTEX PULSE] Relatorio Diario de Operacoes"
$Md += "> **Data:** $(Get-Date -Format 'dd/MM/yyyy') | **Tarefas Cumpridas:** $($RecentTasks.Count)"
...
# 5. Dispara o Feedback Visual (Cyber-Aviso no Windows)
$ps_code = "[Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime] | Out-Null; [Windows.Data.Xml.Dom.XmlDocument, Windows.Data.Xml.Dom, ContentType = WindowsRuntime] | Out-Null; `$xml = New-Object Windows.Data.Xml.Dom.XmlDocument; `$xml.LoadXml(`"<toast><visual><binding template='ToastText02'><text id='1'>Cortex Pulse Gerado</text><text id='2'>$($RecentTasks.Count) atividades executadas pelo sistema hoje. Relatorio disponivel.</text></binding></visual></toast>`"); `$toast = [Windows.UI.Notifications.ToastNotification]::new(`$xml); [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier(`"Nexus Hub`").Show(`$toast)"
