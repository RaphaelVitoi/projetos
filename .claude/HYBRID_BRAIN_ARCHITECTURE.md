...
- **Roteamento Cognitivo e Anti-Exclusividade (Quartetos Dinamicos - Web Premium):**
  ...
  - **Trilha "O Cirurgiao" (Logica/Arquitetura Estrita - @planner, @auditor, @implementor):**
    ...
    - _Terceira & Quarta Vias (APIs):_ **Gemini 2.5 Pro / Flash e OpenRouter Free Tiers** via terminal `nexus` (para automacao material sem custo financeiro). API do Claude atua como ultima linha (4ª opcao) paga.
...
- **A Confiança Mútua:** Quando um modelo lê um arquivo assinado pelo seu parceiro, ele reconhece a competência do outro. Ele NÃO deve reescrever o trabalho fundacional do parceiro (evitando loops de refatoração por "viés de modelo"), mas sim construir _sobre_ ele, agregando sua própria especialidade.
